...
class Sistema:
    ...
    def recuperarMedicamento(self, r):
        return self.__stock[r]
    
    def aumentarStock(self, r, c):
        recover = self.recuperarMedicamento(r)
        cant =  recover.getCantidad()
        recover.setCantidad(int(c) + int(cant))
        
    def disminuirStock(self, r, c):
        recover = self.recuperarMedicamento(r)
        cant = recover.getCantidad()
        recover.setCantidad(int(cant) - int(c))
        
    def cantidadStock(self, r):
        recover = self.recuperarMedicamento(r)
        return recover.getCantidad()
